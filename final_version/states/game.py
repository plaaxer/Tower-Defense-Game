...
from entities.enemy import InimigoFraco, InimigoNormal, InimigoElite, InimigoForte
from entities.turret import Turret, FarmingTurret
    ...
    def create_turret(self, mouse_pos, placing_attack_turret, placing_farming_turret):
        ...
        if self.world.tile_map[mouse_tile_num] == 38:
            ...
            if space_is_free:
                    ...
                    elif placing_farming_turret and self.ControladorLevel.player.tentar_comprar(c.BUY_COST["farming"]):
    ...
    def update(self):
        ...
        if not self.game_over:
            if not self.ControladorLevel.level_comecou:
                ...
                if self.ControladorLevel.level >= 6:
                    for enemy in self.game.enemy_group:
                        enemy.base_speed = 2.5
                if (pg.time.get_ticks() - self.ultimo_spawn_inimigo >= (c.SPAWN_COOLDOWN / self.ControladorLevel.velocidade_jogo + random.randint(-600, 900)) and self.ControladorLevel.inimigos_spawnados < len(self.ControladorLevel.lista_inimigos)):
